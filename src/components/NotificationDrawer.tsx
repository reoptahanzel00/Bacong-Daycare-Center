    ...
    <div className="absolute right-8 top-16 w-96 bg-white rounded-3xl border border-[#E6E4DF] shadow-2xl z-50 overflow-hidden text-left" suppressHydrationWarning>
      ...
      <div className="p-4 border-b border-[#E6E4DF] bg-[#FAF8F5] flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Bell size={18} className="text-[#2F8F8A]" />
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={onClose} className="text-[#9B9B9B] hover:text-[#2B2B2B] p-1 border-none bg-transparent cursor-pointer">
            <X size={16} />
          </button>
        </div>
      </div>
      ...
      <div className="max-h-96 overflow-y-auto p-3 space-y-2.5">
        {notifications.length > 0 ? (
              ...
              >
                <div className="flex items-start gap-2.5">
                  <div className="p-2 rounded-xl bg-white shadow-sm flex-shrink-0">
                    {isAbsence && <AlertTriangle size={16} className="text-[#D32F2F]" />}
                    {isNotice && <Megaphone size={16} className="text-[#8A5D00]" />}
                    {!isAbsence && !isNotice && <Sparkles size={16} className="text-[#2F8F8A]" />}
                  </div>

                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-0.5">
                      <span className="font-bold text-[#2B2B2B] truncate">{n.title}</span>
                      <span className="text-[10px] text-[#9B9B9B]">{n.timestamp}</span>
                    </div>
          ...
          <div className="p-6 text-center text-xs text-[#9B9B9B]">
            <CheckCircle2 size={24} className="mx-auto mb-2 text-[#2F8F8A]" />
      ...
      {/* Drawer Footer */}
      <div className="p-2.5 bg-[#FAF8F5] border-t border-[#E6E4DF] text-center text-[10px] text-[#9B9B9B]">
